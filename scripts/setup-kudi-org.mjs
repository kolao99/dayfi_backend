/**
 * Create (or ensure) the dedicated Kudi Dayfi org + TEST API key.
 *
 * Usage:
 *   npm run build
 *   node -r dotenv/config scripts/setup-kudi-org.mjs
 *
 * Env (optional):
 *   KUDI_ORG_EMAIL=hello@kudi.app
 *   KUDI_ORG_PASSWORD=kudi1234
 */
...
const ORG_ID = 'b0000000-0000-4000-8000-000000000003';
const ORG_SLUG = 'kudi';
const ORG_NAME = 'Kudi';
const MEMBER_EMAIL = (process.env.KUDI_ORG_EMAIL || 'hello@kudi.app').toLowerCase();
const MEMBER_PASSWORD = process.env.KUDI_ORG_PASSWORD || 'kudi1234';
const MEMBER_NAME = 'Kudi Team';
...
async function ensureMember(orgId) {
    ...
    await db.none(
      `UPDATE infra_members
       SET org_id = $1, password_hash = $2, name = $3, role = 'admin',
           first_name = 'Kudi', last_name = 'Team', account_type = 'business',
  ...
  const inserted = await db.one(
    `INSERT INTO infra_members
     ...
     VALUES ($1, $2, $3, $4, 'admin', 'Kudi', 'Team', 'business', TRUE, 1)
     RETURNING id`,
...
async function ensureApiKey(orgId, memberId) {
  ...
  return createApiKey(orgId, 'test', 'Kudi conversational app', {
    memberId,
    orgId,
    email: MEMBER_EMAIL,
    role: 'admin',
  ...
  JSON.stringify(
    {
      ...
      dashboardUrl: 'https://www.dayfi.co/dashboard',
      apiKey: key?.secret || null,
      apiKeyNote: key ? 'new key issued' : 'reuse existing key in kudi/.env or pass --rotate',
    },
    null,
    2
  )
